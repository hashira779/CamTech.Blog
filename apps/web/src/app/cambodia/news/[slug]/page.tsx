...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: {
      canonical: `http://localhost:3000/cambodia/news/${article.slug}`,
    },
    openGraph: {
      title: article.title,
...
export default async function CambodiaArticlePage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  ...
  // Schema.org NewsArticle Structured Data (Section 25)
  const jsonLd = {
    ...
    mainEntityOfPage: {
      "@type": "WebPage",
      "@id": `http://localhost:3000/cambodia/news/${article.slug}`
    }
  };
